...
func TestGetLatencyStats_WithData(t *testing.T) {
	...
	// 先创建一个上游，然后插入请求日志
	_, err := h.store.CreateUpstream("test-up", "https://api.openai.com", nil, 1, "", "round-robin", "api_key", "", false)
...
func TestGetHealthHistory_Empty(t *testing.T) {
	...
	// 需要先创建上游
	up, err := h.store.CreateUpstream("health-up", "https://api.openai.com", nil, 1, "", "round-robin", "api_key", "", false)
	require.NoError(t, err)
...
func TestGetHealthHistory_WithData(t *testing.T) {
	...
	up, err := h.store.CreateUpstream("health-up-2", "https://api.openai.com", nil, 1, "", "round-robin", "api_key", "", false)
	require.NoError(t, err)
...
func TestExportConfig(t *testing.T) {
	...
	// 创建上游（含 API Key）
	_, err := h.store.CreateUpstream("export-up", "https://api.openai.com", []string{"sk-secret-key"}, 1, "", "round-robin", "api_key", "test remark", false)
	require.NoError(t, err)
