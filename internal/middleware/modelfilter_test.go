...
func TestModelFilter_ReloadDeclaredModels(t *testing.T) {
	...
	up, err := s.CreateUpstream("test-up", "https://api.example.com", []string{"sk-test"}, 0, "", "", "", "", false, false, 0)
	require.NoError(t, err)
...
func TestModelFilterMiddleware_Upstream404WithDeclaredModels_Synthesizes(t *testing.T) {
	...
	up, err := s.CreateUpstream("synth-up", "https://api.example.com", []string{"sk-test"}, 0, "", "", "", "", false, false, 0)
	require.NoError(t, err)
