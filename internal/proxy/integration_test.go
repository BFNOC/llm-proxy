...
func TestProbeNow_TriggersProbe(t *testing.T) {
	...
	_, err := s.CreateUpstream("probe-target", healthy.URL, []string{"pk"}, 0, "", "", "", "", false)
	require.NoError(t, err)
...
func TestProbeNow_IncrementalProbes(t *testing.T) {
	...
	healthy := healthyServer(t)
	_, err := s.CreateUpstream("first", healthy.URL, []string{"k1"}, 0, "", "", "", "", false)
	require.NoError(t, err)
	...
	_, err = s.CreateUpstream("second", healthy2.URL, []string{"k2"}, 1, "", "", "", "", false)
	require.NoError(t, err)
...
func TestGetCurrentID_ReturnsZero_AfterProbe(t *testing.T) {
	...
	healthy := healthyServer(t)
	_, err := s.CreateUpstream("test", healthy.URL, []string{"k"}, 0, "", "", "", "", false)
	require.NoError(t, err)
...
func TestProber_DisabledUpstreamsSkipped(t *testing.T) {
	...
	healthy := healthyServer(t)
	up, err := s.CreateUpstream("disabled", healthy.URL, []string{"k"}, 0, "", "", "", "", false)
	require.NoError(t, err)
