...
func TestProber_SelectsHealthyUpstream(t *testing.T) {
	...
	_, err := s.CreateUpstream("primary", healthy.URL, []string{"key-primary"}, 1, "", "", "", "", false)
	require.NoError(t, err)
...
func TestProber_SkipsUnhealthyUpstreams(t *testing.T) {
	...
	_, err := s.CreateUpstream("bad", bad.URL, []string{"key-bad"}, 0, "", "", "", "", false) // priority 0 = higher preference
	require.NoError(t, err)
	_, err = s.CreateUpstream("good", good.URL, []string{"key-good"}, 1, "", "", "", "", false)
	require.NoError(t, err)
...
func TestProber_BothHealthy_BothInList(t *testing.T) {
	...
	_, err := s.CreateUpstream("first", srv1.URL, []string{"key-1"}, 0, "", "", "", "", false)
	require.NoError(t, err)
	_, err = s.CreateUpstream("second", srv2.URL, []string{"key-2"}, 1, "", "", "", "", false)
	require.NoError(t, err)
...
func TestProber_SwitchesOnCurrentFailure(t *testing.T) {
	...
	_, err := s.CreateUpstream("primary", bad.URL, []string{"key-bad"}, 0, "", "", "", "", false)
	require.NoError(t, err)
	_, err = s.CreateUpstream("fallback", good.URL, []string{"key-good"}, 1, "", "", "", "", false)
	require.NoError(t, err)
...
func TestProber_AllUnhealthyKeepsLastActive(t *testing.T) {
	...
	_, err := s.CreateUpstream("bad1", bad1.URL, []string{"key-1"}, 0, "", "", "", "", false)
	require.NoError(t, err)
	_, err = s.CreateUpstream("bad2", bad2.URL, []string{"key-2"}, 1, "", "", "", "", false)
	require.NoError(t, err)
...
func TestProber_AcceptsAuthErrorAsReachable(t *testing.T) {
	...
	_, err := s.CreateUpstream("auth-required", srv.URL, []string{"key-auth"}, 0, "", "", "", "", false)
	require.NoError(t, err)
...
func TestProber_StartContextCancellation(t *testing.T) {
	...
	healthy := healthyServer(t)
	_, err := s.CreateUpstream("primary", healthy.URL, []string{"key-1"}, 0, "", "", "", "", false)
	require.NoError(t, err)
