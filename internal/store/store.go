...
func (s *Store) InsertRequestLogBatch(logs []RequestLog) error {
	...
	stmt, err := tx.Prepare(
		`INSERT INTO request_logs (downstream_key_id, upstream_name, upstream_key_idx, client_ip, ip_region, provider_style, path, status_code, latency_ms, created_at)
		 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
	)
	if err != nil {
		_ = tx.Rollback()
	...
	for _, log := range logs {
		...
		if _, err = stmt.Exec(log.DownstreamKeyID, log.UpstreamName, log.UpstreamKeyIdx, log.ClientIP, log.IPRegion, log.ProviderStyle, log.Path, log.StatusCode, log.LatencyMs, createdAt); err != nil {
			_ = tx.Rollback()
...
func (s *Store) QueryLogs(keyID int64, from, to time.Time, limit int) ([]RequestLog, error) {
	query := `SELECT id, downstream_key_id, upstream_name, upstream_key_idx, client_ip, ip_region, provider_style, path, status_code, latency_ms, created_at
	...
	for rows.Next() {
		var rl RequestLog
		if err := rows.Scan(&rl.ID, &rl.DownstreamKeyID, &rl.UpstreamName, &rl.UpstreamKeyIdx, &rl.ClientIP, &rl.IPRegion, &rl.ProviderStyle, &rl.Path, &rl.StatusCode, &rl.LatencyMs, &rl.CreatedAt); err != nil {
